...
//			GlobalFunc.getInstance().Deebug("now selectedIndex = " + TopicList.selectedIndex);
		...
		this.TTS_doBorkFunction(this.TopicListHolder.List_mc.selectedEntry.text,"DIALOGUE");
		this.checkStart = false;
        this.xx = 0;
        this.timer = setInterval(this,"TTS_checkForSpeechDone",200);
	}
	...
	function TTS_checkForSpeechDone()
	{
		var done: String = this.TTS_doBorkFunction("","CHECK_DONE");
		if(done == "NO")
		{
			this.checkStart = true;
		}
		if(done == "YES" || this.xx > 50)
		{
			this.checkStart = false;
			this.xx = 0;
			clearInterval(this.timer);
			this.doTopicClickedReal();
		}
		this.xx = this.xx + 1;
	}
	function doTopicClickedReal()
	{
	...
	function TTS_doSendTextForSpeech()
	{
		this.TTS_doBorkFunction(this.TopicListHolder.List_mc.selectedEntry.text,"DIALOGUE_CLICK");
	}
	function TTS_doBorkFunction(text, type)
	{
		return _global.skse.plugins.FuzRoBork.BorkFunction(text,type);
	}
}
